...
entities_file_path = (
    Path(__file__).resolve().parents[3] / "suturo_rasa" / "entities.yml"
)
...
def phonic_match_terms(
    terms, allowed_entities, similarity_fn, threshold=0.9, min_result=5, max_attempts=16
):
    """
...
def nounDictionary(text):
    ...
    nouns_to_replace = collect_nouns(
        doc, allowed_entities, excluded_term=names_to_replace
    )
    names = phonic_match_terms(
        names_to_replace, allowed_entities, double_metaphone_similarity
    )
    dictionary = phonic_match_terms(
        nouns_to_replace, allowed_entities, double_metaphone_similarity
    )
...
def calculate_phonetic_score(words1, words2):
    ...
    word_count_penalty = max(
        WORD_COUNT_PENALTY_CAP, 1 - (word_count_diff / WORD_COUNT_PENALTY_DIVISOR)
    )
